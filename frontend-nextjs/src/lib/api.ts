...
export const authApi = {
  ...
  // GET /auth/profile/ — deduplicated: concurrent calls share one in-flight request
  me: (() => {
    let inflight: Promise<UserProfile> | null = null;
    return () => {
      if (!inflight) {
        inflight = request<UserProfile>(AUTH, "/auth/profile/").finally(() => {
          // Clear after 5 s so the next page navigation re-fetches fresh data
...
export interface CloudFile {
  id: string;
  owner_id?: number;
  original_name: string;   // field thực từ file-service
  name?: string;           // alias cho frontend dùng
  blob_name?: string;
...
export const shareApi = {
  ...
  // GET /share/<token>/ (no auth needed)
  publicShare: (token: string) => request<PublicShareFile>(SHARE, `/share/${token}/`),
};

export const adminApi = {
  users: () => request<AdminUserRow[]>(AUTH, "/admin/users/"),
};
