        ...
        <section className="relative z-10 mx-auto max-w-6xl px-6 py-10">
          ...
          {view === "grid" && (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              <AnimatePresence mode="popLayout">
                {filtered.map((f, i) => (
                  <FileCard key={f.id} file={f} index={i} onDeleted={load} onPreview={() => setPreview(f)} />
                ))}
              </AnimatePresence>
            </div>
          )}
          ...
          {view === "list" && (
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
              <AnimatePresence mode="popLayout">
                {filtered.map((f, i) => (
                  <FileRow key={f.id} file={f} index={i} onDeleted={load} onPreview={() => setPreview(f)} isLast={i === filtered.length - 1} />
                ))}
              </AnimatePresence>
            </div>
          )}
...
function FileCard({ file, index, onDeleted, onPreview }: {
  ...
  const { Icon, color, type } = getFileIcon(file.name);
        ...
        {isImage && file.url
          ...
          : (
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs font-mono text-[var(--muted-foreground)]">.{file.name.split(".").pop()?.toUpperCase()}</span>
            </div>
          )}
        {/* Hover overlay */}
